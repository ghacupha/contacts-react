...
export const getSearchEntities: ICrudSearchAction<IContact> = (query, page, size, sort) => ({
  type: ACTION_TYPES.SEARCH_CONTACTS,
  payload: axios.get<IContact>(`${apiSearchUrl}?query=${query}${sort ? `&page=${page}&size=${size}&sort=${sort}` : ''}`),
});
