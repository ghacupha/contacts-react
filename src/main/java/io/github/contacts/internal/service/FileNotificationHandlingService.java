...
import io.github.contacts.internal.fileProcessing.FileUploadProcessorChain;
import io.github.contacts.internal.util.TokenGenerator;
...
import static io.github.contacts.internal.AppConstants.PROCESSED_TOKENS;
...
public class FileNotificationHandlingService implements HandlingService<FileNotification> {
    ...
    private final ContactsMessageTokenService messageTokenService;
    private final ContactsMessageTokenMapper messageTokenMapper;
    private final ContactsFileUploadService fileUploadService;
    private final FileUploadProcessorChain fileUploadProcessorChain;

    public FileNotificationHandlingService(TokenGenerator tokenGenerator, ContactsMessageTokenService messageTokenService, ContactsMessageTokenMapper messageTokenMapper, ContactsFileUploadService fileUploadService, FileUploadProcessorChain fileUploadProcessorChain) {
    ...
    public void handle(FileNotification payload) {
        ...
        // @formatter:off
        ContactsMessageToken messageToken = new ContactsMessageToken()
            .tokenValue(token)
        ...
        ContactsFileUploadDTO fileUpload =
        ...
        if (!PROCESSED_TOKENS.contains(payload.getMessageToken())) {
            ...
            List<ContactsFileUploadDTO> processedFiles = fileUploadProcessorChain.apply(fileUpload, payload);
        ...
        ContactsMessageTokenDTO dto = messageTokenService.save(messageTokenMapper.toDto(messageToken));
